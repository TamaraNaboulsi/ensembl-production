...
sub run {
  ...
  while(my $chr = shift @chrs) {
     $self->warning("Processing..................chr: $chr->name()");
...
sub map_data {
  ...
  my @decoded_segments;
  #my $projection = $slice->project($self->param('level'), $self->param('assembly_default'));
  my $projection = $slice->project('contig'); #, $self->param('assembly_default'));
