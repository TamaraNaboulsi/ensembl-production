...
#use lib path(__FILE__)->absolute->parent(2)->stringify;

use Bio::EnsEMBL::Production::Pipeline::Webdatafile::lib::GenomeLookup;
#use ProjectPath qw(variants_path);
...
sub get_variants_dir_path {
  my ($self) = @_;
  #if (!$self->genome) {
  #  $self->genome(GenomeLookup->new()->get_genome($self->genome_id));
  #}
  return $self->variant_path; #variants_path($self->genome);
}
